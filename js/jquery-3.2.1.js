/ *!
 * Библиотека jQuery JavaScript v3.2.1
 * https://jquery.com/
 *
 * Включает Sizzle.js
 * https://sizzlejs.com/
...
var support = {};
	...
	function DOMEval (code, doc) {
		...
		var script = doc.createElement ("сценарий");
...
вар
	...
	// Убедитесь, что мы обрезаем BOM и NBSP
...
jQuery.fn = jQuery.prototype = {
	...
	// (возвращение нового согласованного набора элементов)
...
jQuery.extend = jQuery.fn.extend = function () {
	параметры var, имя, источник, копия, copyIsArray, клонировать,
		цель = аргументы [0] || {},
		я = 1,
...
jQuery.extend ({
	...
	isFunction: function (obj) {
		return jQuery.type (obj) === "функция";
	},
	...
	isNumeric: function (obj) {
		...
		// который может быть приведен к конечным числам (gh-2662)
		var type = jQuery.type (obj);
		return (type === "number" || type === "string") &&

			// parseFloat NaNs числовое приведение ложных срабатываний ("")
	...
	// Microsoft забыла указать префикс производителя (# 9572)
	camelCase: function (string) {
		вернуть строку.replace (rmsPrefix, "ms-") .replace (rdashAlpha, fcamelCase);
	},
	...
	// Привязываем функцию к контексту, при желании частично применяя любые
	// аргументы.
 ...
 * https://sizzlejs.com/
 *
 * Авторские права jQuery Foundation и других участников
 * Выпущено по лицензии MIT
...
вар я,
	...
	booleans = "проверено | выбрано | асинхронно | автофокус | автозапуск | элементы управления | отложено | отключено | скрыто | ismap | loop | multiple | open | readonly | required | scoped",
	...
	attributes = "\\ [" + пробел + "* (" + идентификатор + ") (?:" + пробел +
		// Оператор (захват 2)
		"* ([* ^ $ |! ~]? =)" + пробел +
	...
	// Начальные и конечные пробельные символы без экранирования, захват некоторых непробельных символов, предшествующих последнему
	rwhitespace = новый RegExp (пробел + "+", "g"),
	rtrim = new RegExp ("^" + пробел + "+ | ((?: ^ | [^ \\\\]) (?: \\\\.) *)" + пробел + "+ $", "g "),
	...
	matchExpr = {
		...
		"CHILD": new RegExp ("^ :( only | first | last | nth | nth-last) - (child | of-type) (?: \\ (" + пробел +
			"* (четный | нечетный | (([+ -] |) (\\ d *) n |)" + пробел + "* (?: ([+ -] |)" + пробел +
			«* (\\ d +) |))» + пробел + «* \\) |)», «i»),
		...
		"needsContext": new RegExp ("^" + пробел + "* [> + ~] |: (четный | нечетный | eq | gt | lt | nth | первый | последний) (?: \\ (" +
			пробел + "* ((?: - \\ d)? \\ d *)" + пробел + "* \\) |) (? = [^ -] | $)", "i")
	},
	...
	funescape = function (_, escape, escapedWhitespace) {
		...
		вернуть высокий! == высокий || сбежалWhitespace?
			сбежал:
			высокий <0?
				// Кодовая точка BMP
...
пытаться {
	...
	push = {применить: arr.length?
		...
		function (target, els) {
			push_native.apply (цель, slice.call (els));
		}:
...
function Sizzle (селектор, контекст, результаты, начальное число) {
	...
	if (! seed) {
		...
		if ((context? context.ownerDocument || context: primaryDoc)! == документ) {
		...
		if (documentIsHTML) {
			...
			если (support.qsa &&
				...
				} else if (context.nodeName.toLowerCase ()! == "объект") {
